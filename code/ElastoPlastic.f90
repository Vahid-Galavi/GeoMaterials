...
public:: ElasticPerfectlyPlastic
public:: GetParamAndUnitElasticPerfectlyPlastic
public:: GetParamCountElasticPerfectlyPlastic
public:: GetStateVarCountElasticPerfectlyPlastic
public:: GetModelNameElasticPerfectlyPlastic
...
!---------------------------------------------------------------------------------------------------------
subroutine ElasticPerfectlyPlastic(IDTask, isUndr,                    &
...
end subroutine ElasticPerfectlyPlastic
  ...
  !do iSub= 1, nSub
    ...
    if (.not.isConverged) then
      ...
      if (ABORT_WHEN_NOT_CONVERGED .or. WRITE_DEBUG_FILE_WHEN_NOT_CONVERGED) then
        ...
        call WriteInDebugFile('Sig0       :' // trim(string(Sig0,   1,N_STRESS_VECTOR)),          ALWAYS_LEVEL)
...
if (IsDebug) then
  call calPrincipalStresses(Sig0, prSig0)
  call calAllYieldFs(props, -prSig, fYieldEnd)
  ...
  call WriteInDebugFile('Sig0       :' // trim(string(Sig0,    1, N_STRESS_VECTOR)))
...
double precision, intent(in):: GCurrent
...
if (fYield(INDEX_MOHR_COULOMB_YIELD) > getTolerance(IDYieldFunc, prSig)) then
  ! cone
  if (IsDebug) then
    call WriteInDebugFile('before ConeYieldSurface 665')
  endif
...
! local variables
integer, parameter :: INT_DEBUG_LOCAL = -1
integer :: IDYieldFunc, iter
double precision:: f, dLamda, dFdLamda
...
if (abs(f) < getTolerance(IDYieldFunc, prSig)) then
  isConverged = .true.
...
do while (iter < ITER_MAX .and. abs(f) > getTolerance(IDYieldFunc, prSig))
  ...
  if (IsDebug) then
    ...
    call WriteInDebugFile('dFdLamda         : ' // trim(string(dFdLamda)))
    call WriteInDebugFile('dLamda           : ' // trim(string(dLamda)))
    call WriteInDebugFile('f / dFdLamda     : ' // trim(string(f / dFdLamda)))
  endif
  ...
  if (abs(f) > DIVERGENCE_LIMIT) EXIT
...
if (abs(f) < getTolerance(IDYieldFunc, prSig)) isConverged = .true.
...
double precision, dimension(N_CROSS_YIELD_SURFACES) :: f
...
do while (iter < ITER_MAX .and. &
  ...
  ! AInverse*f
  call MatrixToVectorProd(dFdLamdaInverse, f, 2, ddLamda)
  ...
  do i=1,N_CROSS_YIELD_SURFACES
    ...
    if (abs(f(i)) > DIVERGENCE_LIMIT) then
      isConverged = .false.
      RETURN
    endif
  enddo
...
if (IsDebug) then
  call WriteInDebugFile('iter     : ' // trim(string(iter)))
  call WriteInDebugFile('f        : ' // trim(string(f,1,2)))
endif

if (abs(f(FRICTION)) < getTolerance(IDYieldFunc(FRICTION), prSig) .and. &
    abs(f(TENSION))  < getTolerance(IDYieldFunc(TENSION),  prSig)      ) isConverged = .true.
...
!--------------------------------------------------------------------------------------------------
integer function GetParamCountElasticPerfectlyPlastic() result(res)
implicit none

res = NPARAMS

end function GetParamCountElasticPerfectlyPlastic

!--------------------------------------------------------------------------------------------------
integer function GetStateVarCountElasticPerfectlyPlastic() result(res)
implicit none

res = NSTATS

end function GetStateVarCountElasticPerfectlyPlastic
...
!--------------------------------------------------------------------------------------------------
subroutine GetParamAndUnitElasticPerfectlyPlastic(iParam, ParamName, Units)
!
...
end subroutine GetParamAndUnitElasticPerfectlyPlastic

!--------------------------------------------------------------------------------------------------
subroutine GetModelNameElasticPerfectlyPlastic(ModelName)
use ModGlobalConstants
implicit none
character (Len= * ) ModelName

ModelName= 'ElasticPerfectlyPlastic'

end subroutine GetModelNameElasticPerfectlyPlastic

end module ElasticPerfectlyPlastic_Module
